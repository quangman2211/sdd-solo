...
chk "new: tạo #1, file + dòng mục lục + commit giữ chỗ (exit $R)" '[ $R = 0 ] && [ -f notes/hoi-dap/phieu/001-uc-001-gui-trung.md ] && grep -q "^| #1 |" notes/hoi-dap/hoi-dap.md && git log -1 --format=%s | grep -q "phiếu #1 placeholder"'
...
S phieu.sh muc-luc
chk "muc-luc ở main: khớp mục lục/file (exit $R)" '[ $R = 0 ] && has "the index and the files match"'
# mục lục lệch → đỏ
cp notes/hoi-dap/phieu/001-uc-001-gui-trung.md notes/hoi-dap/phieu/001-trung.md
S phieu.sh muc-luc
chk "hai file cùng số → đỏ (exit $R)" '[ $R = 1 ] && has "two files with the same number"'
