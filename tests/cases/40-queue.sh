...
chk "add hai việc, commit --only (exit $R)" '[ $R = 0 ] && grep -c "^| [bc]-uc-001-p1 |" notes/hang-doi.md | grep -q "^2$" && git log -1 --format=%s | grep -q "^chore(sdd): queue"'
S queue.sh next
...
S queue.sh next
chk "làn spec sức chứa 1 đang bận → next rỗng" 'has "nothing can go out right now"'
S queue.sh done b-uc-001-p1
chk "done không có KETQUA → đỏ (exit $R)" '[ $R = 1 ] && has "has no KETQUA yet"'
S role.sh --ketqua b-uc-001-p1 ket=chan hoi=#2
S queue.sh done b-uc-001-p1
chk "done khi KETQUA là chan → đỏ (exit $R)" '[ $R = 1 ] && has "ket=chan, not xong"'
...
S queue.sh board
chk "board: đang 2 vai D, sẵn sàng c, chờ d-uc-003 (exit $R)" '[ $R = 0 ] && has "lane code   2/2 active" && hasE "d-uc-003-p1 +needs"'
# xong không neo là đỏ
rep notes/hang-doi.md "| xong | $H |" "| xong | - |"
S queue.sh board
chk "xong mà Neo trống → board đỏ (exit $R)" '[ $R = 1 ] && has "done with an empty Anchor"'
...
chk "worktree phụ add → từ chối (exit $R)" '[ $R = 1 ] && printf "%s" "$O" | grep -q "only the coordinator in the main checkout"'
