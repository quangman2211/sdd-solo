...
chk "gate-check sau gate-pass vẫn xanh (exit $R)" '[ $R = 0 ] && has "the gate-pass commit"'
code_uc1
...
S gate-check.sh UC-001
chk "gate-check sau close: chỉ đỏ status implemented, không đỏ §9" 'has "status is already implemented" && ! has "changed BEHAVIOUR"'
