...
s = s.replaceAll('Cho: <one line per role: - **B:** … · - **D:** … · - **T:** …>', 'Cho:\n- **B:** áp F1…F24 [neo: UC-001 ## Main Flow bước 2]\n- **D · T:** không có việc');
let body = '';
...
S phieu.sh close 1
chk "khai 25 mà file có 24 → đỏ, kê thiếu #25 (exit $R)" '[ $R = 1 ] && has "declares 25 entries but the file counts 24"'
app "$F" "- F25 phát hiện 25 [neo: Main 3] → sửa"
S phieu.sh close 1
chk "đủ 25 nhưng B chưa có KETQUA → đỏ (exit $R)" '[ $R = 1 ] && has "role B has work in For: but no KETQUA"'
