...
S role.sh B
chk "role.sh B ghi dấu vào .git/sdd-role, in hợp đồng" '[ "$(cat .git/sdd-role)" = B ] && has "may write: specs/**"'
S role.sh --xem
...
chk "khong: B ghi src → cho qua (exit $R), nhắc 'chỗ này của vai D'" '[ $R = 0 ] && grep -q "this belongs to role D" "$W/hookerr.txt"'
...
chk "nhanh-vai: B ghi src → CHẶN (exit $R)" '[ $R != 0 ] && grep -q "blocked (vai_bat_buoc=nhanh-vai)" "$W/hookerr.txt"'
...
chk "SDD_ROLE=T → src bị chặn, thông điệp nói của vai D (exit $R)" '[ $R != 0 ] && grep -q "this belongs to role D" "$W/hookerr.txt"'
...
chk "R.commit=khong → chặn (exit $R)" '[ $R != 0 ] && grep -q "may not commit" "$W/hookerr.txt"'
...
chk "D ghi uc_test_dir trong worktree → chặn, nói của vai T (exit $R)" '[ $R != 0 ] && grep -q "this belongs to role T" "$W/hookerr.txt"'
...
chk "không có .sdd/roles → hook im lặng, qua (exit $R)" '[ $R = 0 ] && ! grep -qiE "vai|role" "$W/hookerr.txt"'
