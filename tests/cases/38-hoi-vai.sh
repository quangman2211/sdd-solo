...
chk "hoi D: tạo sổ từ khuôn, mục HỎI-D1 (exit $R)" '[ $R = 0 ] && [ -f notes/hoi-dap/hoi-D.md ] && grep -q "^### HỎI-D1 · Notify.workingHours" notes/hoi-dap/hoi-D.md && grep -q "^# ASK from role D · code" notes/hoi-dap/hoi-D.md'
S phieu.sh hoi D "Verb receive tính là nhận được không"
chk "hoi D lần hai: HỎI-D2" 'grep -q "^### HỎI-D2 " notes/hoi-dap/hoi-D.md'
S hoi-check.sh D
chk "hoi-check: bốn ô còn khuôn → đỏ (exit $R)" '[ $R = 1 ] && has "is empty or still a template"'
node - <<'JS'
const fs = require('fs');
const p = 'notes/hoi-dap/hoi-D.md';
let s = fs.readFileSync(p, 'utf8');
s = s.replaceAll('- **Nguồn:** <file:section already looked up>', '- **Nguồn:** UC-001 ## Open Questions · RULE-001 tham số phút');
s = s.replaceAll('- **Chặn không:** <chặn/không chặn> — <why>', '- **Chặn không:** không chặn — mọi ngày như nhau cho tới khi spec nói');
...
JS
S hoi-check.sh D
chk "đủ bốn ô → xanh (exit $R)" '[ $R = 0 ] && has "2 ASK-D entries, all four boxes filled"'
...
S hoi-check.sh D
chk "cổng UC-001 đã mở mà đích trỏ thân UC → đỏ (exit $R)" '[ $R = 1 ] && has "the target points into the UC body"'
...
chk "--lich-su bắt commit sửa dòng HỎI đã có (exit $R)" '[ $R = 1 ] && has "edited an existing"'
