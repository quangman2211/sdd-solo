...
chk "commit-msg chặn feat(UC-001) khi chưa có marker (exit $R)" '[ $R != 0 ] && grep -q "has not passed the DoR gate" "$W/hookerr.txt"'
git reset -q
cmv "docs: không ID"; R=$?
chk "commit-msg chặn commit đụng code không có ID (exit $R)" '[ $R != 0 ] && grep -q "must carry an ID" "$W/hookerr.txt"'
...
chk "pre-commit chặn trộn spec + code (exit $R)" '[ $R != 0 ] && grep -q "mixes spec" "$W/hookerr.txt"'
