...
- Mỗi ngoại lệ khai trong `## Exceptions` của UC phải xuất hiện ở đây, làm nhãn mũi tên `|E# tên ngoại lệ|`. Thiếu = có ngoại lệ chưa vẽ đường đi.
- Mỗi nhãn ngoại lệ ở đây phải có thật trong `## Exceptions`. Không dán nhãn cho ID chưa tồn tại.
- Đừng đặt id node bằng chữ `E` kèm số cho việc khác — dạng đó luôn được đọc là ngoại lệ. Node kết bình thường dùng `P#`, node kết của ngoại lệ dùng `X#`.
