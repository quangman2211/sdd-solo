...
| `--xem` | the current role: `$SDD_ROLE` → the worktree marker → the branch pattern | if none can be inferred, nothing is being enforced. The name is the **first token**, so a pane opened with `SDD_ROLE="C review opus"` is the role `C` (8.6.0) |
...
| `--ketqua <key> ket=xong neo=<hash> [dat=<n>/<m>]` | writes one KETQUA line into `$(git rev-parse --git-common-dir)/sdd-ketqua/<key>.txt` — shared by every worktree, outside git | the agent calls this **before** sending its message back; `ket=xong` requires a real `neo`, `ket=chan` requires `hoi=`. When the message is lost, the file is still there (P-26). `dat=<passed>/<total>` (8.9.0) is the one number a coordinator reads off the board from a job that COUNTS something — a measurement run, a test sweep. It is REPORTED, not gated: `done` prints a shortfall and still closes the row, because what the measurement found is a finding to file, not a gate to fail |
