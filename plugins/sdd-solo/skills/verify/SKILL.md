...
## B. `/sdd-solo:verify` with no ID — the tree sweep

A different run altogether: it reads the whole spec tree instead of re-reading one UC or one change. Read
`references/tree-sweep.md` and follow it; part A above does not apply. Full path
`${CLAUDE_PLUGIN_ROOT}/skills/verify/references/tree-sweep.md` (if the variable is not substituted:
`find ~/.claude/plugins -type f -name tree-sweep.md -path '*sdd-solo*' | head -1`).
