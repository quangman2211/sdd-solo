...
5. Create:
   - `specs/<core|craft>/br-###/use-cases/$1-<slug>/` from `${CLAUDE_PLUGIN_ROOT}/templates/skel/use-case/` (copy `UC-000.md` → `$1.md`, `UC-000.flow.md` → `$1.flow.md`, `UC-000.sequence.md` → `$1.sequence.md`, `UC-000.trace.md` → `$1.trace.md`). **Do NOT create `screens/`** (8.10.0): a folder for drawings is an empty drawer until somebody draws, and measured at runxops 10 of 21 UCs had one holding nothing but its README. Whoever draws creates it at step ⑤, from `${CLAUDE_PLUGIN_ROOT}/templates/skel/use-case/screens/README.md`. The gate reads the `## Screens` TABLE in the UC body, never the folder. Replace every `UC-000` with `$1`, `Last updated` with today, `Status: draft`.
