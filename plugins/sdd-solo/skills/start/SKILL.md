...
3. Context (`$2`): nếu không có, liệt kê `specs/contexts/*/` (bỏ `_template`) và hỏi user chọn. Nếu context chưa có → hỏi có tạo từ `_template/` không.
...
5. Tạo:
   - `specs/contexts/<ctx>/use-cases/$1-<slug>/` từ `specs/contexts/_template/use-cases/UC-000-template/` (copy `UC-000.md` → `$1.md`, `UC-000.sequence.md` → `$1.sequence.md`, `screens/README.md`). Thay mọi `UC-000` thành `$1`, `<ctx>` thành context, `Last updated` thành hôm nay, `Status: draft`.
