...
① `/sdd-solo:start UC-###` → ② **điền nội dung UC cùng user** (Actor · Trigger · Preconditions · Main Flow — bước hiển thị nêu SCR-ID · Alternative · Exceptions · Postconditions) → ③ user viết RULE (rules.md, DMN nếu cần), **`entities.md` + `glossary.md` của context**, và AC → ④ vẽ flow mermaid trong `UC-###.flow.md` → ⑤ Claude Design theo `.sdd/prompts/design-brief.md` → ⑥ đối chiếu SCR ↔ E# ↔ state → ⑦ `/sdd-solo:adversarial` (3 vai, session mới) → ⑧ **đọc lại bằng đầu chưa neo**: `/sdd-solo:verify UC-###` (subagent; bắt buộc từ 6.0.0 — không còn cửa "đóng máy đọc lại buổi sau") → ⑨ `/sdd-solo:gate` (đỏ/xanh) → ⑩ `/sdd-solo:design` — sinh `design.md` + `tasks.md` trong thư mục UC, user đọc, bắt lệch → ⑪ viết code theo `tasks.md` → ⑫ test theo AC → ⑬ self-review 5 câu → ⑭ `/sdd-solo:close` → `/sdd-solo:state`.
